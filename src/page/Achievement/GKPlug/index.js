...
class GKPlug extends Component {
  state = {
    data: [],
    dataUser: [],
  };
  ...
  componentDidMount() {
    Axios.get("http://api.mapid.io/blog/get_list_docs_public/blog")
      .then((res) => {
        this.setState({ data: res.data[16] });
    ...
    Axios.get("http://api.mapid.io/blog/get_list_docs_public/blog")
      .then((res) => {
        this.setState({ dataUser: res.data[16].authors[0].user });
        console.log("GET data user berhasil", res.data[16].authors[0].user);
      })
      .catch((err) => {
        console.log(err.message);
      });
  ...
  render() {
    return (
      <>
        <Sidebar />
        <div className="titleWrapergk bgphotogk">
        ...
        <div className="documentContainer">
          <div className="profile">
            ...
            <div className="content2u">{this.state.data.title}</div>
