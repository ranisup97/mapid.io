...
class MapidDufan extends Component {
  state = {
    data: [],
    dataUser: [],
  };
  componentDidMount() {
    Axios.get("http://api.mapid.io/blog/get_list_docs_public/blog")
      .then((res) => {
        this.setState({ data: res.data[18] });
    ...
    Axios.get("http://api.mapid.io/blog/get_list_docs_public/blog")
      .then((res) => {
        this.setState({ dataUser: res.data[18].authors[0].user });
        console.log("GET data user berhasil", res.data[18].authors[0].user);
      })
      .catch((err) => {
        console.log(err.message);
      });
